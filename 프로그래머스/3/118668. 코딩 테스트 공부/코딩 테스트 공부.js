...
function solution(alp, cop, problems) {
  ...
  const goal = problems
    .map((p) => p.slice(0, 2))
    .reduce(
      ([maxA, maxB], [a, b]) => [Math.max(maxA, a), Math.max(maxB, b)],
      [alp, cop]
    ) // 초기 위치를 초기 능력치 이후로 보정

  // [alp][cop] 형태로 동작하는 2차원 배열 만들어보자
  const stat = Array.from(new Array(goal[0] + 1), (_, i) =>
    new Array(goal[1] + 1).fill(false)
  ) // alp, cop으로 배열에 접근할 수 있도록 길이 설정
    ...
    for (let j = cop; j < goal[1] + 1; j++) {
      ...
      problems.forEach((problem) => {
        ...
        if (solvable) {
          ...
          stat[target_alp][target_cop] = Math.min(
            stat[target_alp][target_cop],
            stat[i][j] + cost
          )
        }
      })
